...
describe('PasswordChecker test suite', () => {
  ...
  it('Password with more than 8 chars is valid', () => {
    const actual = sut.checkPassword('12345678Aaa');
  ...
  it('Password with upper case letter is valid', () => {
    const actual = sut.checkPassword('abcdE');
  ...
  it('Admin password with no number is invalid', () => {
    const actual = sut.checkAdminPassword('ABCDefghi');
    expect(actual.reasons).toContain(PasswordErrors.NO_NUMBER);
    expect(actual.valid).toBe(false);
  });
  
  it('Admin password with number is valid', () => {
    const actual = sut.checkAdminPassword('ABCDefghi1');
